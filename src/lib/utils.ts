...
export function getImageUrl(path: string | null | undefined, size: "poster" | "backdrop" = "poster"): string {
  ...
  const placeholderPoster = "https://placehold.co/342x513?text=No+Image";
  const placeholderBackdrop = "https://placehold.co/1280x720?text=No+Image";
  ...
  if (path.startsWith('http')) {
    ...
    if (path.includes('cdn.movieofthenight.com')) {
      ...
      if (size === "poster") {
        // If it already contains the size we want, return it
        if (path.includes('/480.jpg')) {
          return path;
        }

        // Otherwise, try to replace the size
        return path.replace(/\/\d+\.jpg/, '/480.jpg');
      } else {
        ...
        if (basePath && basePath.length > 2) {
          return `${basePath[1]}/backdrop/horizontal/1080.jpg${basePath[2].substring(basePath[2].indexOf('?'))}`;
        }

        // If we can't extract the base path, return the original
        return path;
      }
    }
  ...
  if (path.startsWith('/')) {
    ...
    return `${baseUrl}${defaultSize}${path}`;
  }
